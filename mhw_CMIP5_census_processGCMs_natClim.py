...
for model in models:
            ...
            for ens in range(NENS[model][exp]):
                ...
                for j in range(Y):
                    ...
                    if np.logical_not(np.isfinite(sst[exp][which_pre2100,j,ens].sum())) + ((sst[exp][which_pre2100,j,ens]<-0).sum()>0): # check for land, ice
                        continue
                    # MHW detection
                    if (exp == 'hist') + (exp == 'rcp45') + (exp == 'rcp85'):
                        if ens > NENS[model]['histNat']-1:
                            ens_clim = NENS[model]['histNat']-1
                        else:
                            ens_clim = ens
                    else:
                        ens_clim = ens
                    mhws, clim = mhw.detect(t[model][exp][which_pre2100], sst[exp][which_pre2100,j,ens], climatologyPeriod=climPeriod, alternateClimatology=[t[model]['histNat'], sst['histNat'][:,j,ens_clim]])
    ...
    # Save data
    outfile = outheader + '/mhw_census_' + model + '_climPeriod_' + str(climPeriod[0]) + '_' + str(climPeriod[1]) + '_natClim'
